...
class VesselAvailabilityController extends Controller
{
    ...
    public function index(Request $Request)
    {
        ...
        $NumberOfVessels_IDLE = VesselAvailability::select('Vessel')->where('Status', 'IDLE')
                                ...
                                // ->orWhere('EndDate', '<', date('Y-m-d'))
                                ->groupBy('Vessel')->get(); 
        $NumberOfVessels_IDLE = VesselAvailability::select('Vessel')->join('vessels_vessel_information', 'vessels_vessel_information.VesselName', '=', 'vessel_availabilities.Vessel')->where('EndDate', '<', date('Y-m-d'))->groupBy('Vessel')->get();
