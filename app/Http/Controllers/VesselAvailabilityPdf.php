...
{ 
    public function vessel_availability_report(Fpdf $fpdf, Request $Request) {  
        ...
        if (isset($Request->Month) AND isset($Request->Vessel)) { 
            ...
            $fpdf->Cell(158.5, 5, '', 0);
            $fpdf->Cell(31.7, 5, 'T: s' . collect($TotalHoursWorked)->sum() == 0 ? collect($TotalMinutesWorked)->sum() . ' min(s) : ' : collect($TotalHoursWorked)->sum() . ' hour(s) : ' . collect($TotalDaysWorked)->sum() . ' day(s)', 1);
            $fpdf->Ln(15);
            $fpdf->SetFont('Arial', 'B', 14);  
