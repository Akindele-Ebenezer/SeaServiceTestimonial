  ...
  <div class="chart-4-wrapper">
    ...
    <ul class="pieID legend">
      ...
      <li>
        ...
        <span class="InspectionCount"><small>{{ round(\Carbon\CarbonInterval::hours(collect($TotalHoursArr_INSPECTION)->sum())->totalDays) . ' day(s)' }} <span class="Hide">{{ (collect($TotalMinutesArr_INSPECTION)->sum() / 60) }}</span> : {{ ((collect($TotalHoursArr_INSPECTION)->sum() == 0) ? collect($TotalMinutesArr_INSPECTION)->sum() . ' min(s)' : collect($TotalHoursArr_INSPECTION)->sum() . ' hour(s)') }}</small></span>
        @else
      ...
      <li>
        ...
        <span class="MaintenanceCount"><small>{{ round(\Carbon\CarbonInterval::hours(collect($TotalHoursArr_MAINTENANCE)->sum())->totalDays) . ' day(s)' }} <span class="Hide">{{ (collect($TotalMinutesArr_MAINTENANCE)->sum() / 60) }}</span> : {{ ((collect($TotalHoursArr_MAINTENANCE)->sum() == 0) ? collect($TotalMinutesArr_MAINTENANCE)->sum() . ' min(s)' : collect($TotalHoursArr_MAINTENANCE)->sum() . ' hour(s)') }}</small></span>
        @else
      ...
      <li>
        ...
        <span class="BreakdownCount"><small>{{ round(\Carbon\CarbonInterval::hours(collect($TotalHoursArr_BREAKDOWN)->sum())->totalDays) . ' day(s)' }} <span class="Hide">{{ (collect($TotalMinutesArr_BREAKDOWN)->sum() / 60) }}</span> : {{ ((collect($TotalHoursArr_BREAKDOWN)->sum() == 0) ? collect($TotalMinutesArr_BREAKDOWN)->sum() . ' min(s)' : collect($TotalHoursArr_BREAKDOWN)->sum() . ' hour(s)') }}</small></span>
        @else
