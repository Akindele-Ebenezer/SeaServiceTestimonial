...
<div class="form-1 chart-4 {{ (isset($_GET['FilterValue']) || isset($_GET['page']) || isset($_GET['Vessel_FILTER'])) ? 'Hide' : '' }}" style="display: {{ isset($_GET['Chart4']) ? 'flex !important' : '' }}">
  <div class="chart-4-wrapper">
    ...
    <ul class="pieID legend">
      <li>
        <em>Docking</em>
        <span class="DockingCount">{{ isset($_GET['Chart4']) ? count($MonthlyVessel_DOCKING_STATS) : 1 }}</span>
      </li>
      <li>
        <em>Bunkering</em>
        <span class="BunkeringCount">{{ isset($_GET['Chart4']) ? count($MonthlyVessel_BUNKERY_STATS) : 1 }}</span>
      </li>
      <li>
        <em>Inspection</em>
        <span class="InspectionCount">{{ isset($_GET['Chart4']) ? count($MonthlyVessel_INSPECTION_STATS) : 1 }}</span>
      </li>
      <li>
        <em>Maintenance</em>
        <span class="MaintenanceCount">{{ isset($_GET['Chart4']) ? count($MonthlyVessel_MAINTENANCE_STATS) : 1 }}</span>
      </li>
      <li>
        <em>Breakdown</em>
        <span class="BreakdownCount">{{ isset($_GET['Chart4']) ? count($MonthlyVessel_BREAKDOWN_STATS) : 1 }}</span>
      </li>
    </ul>
